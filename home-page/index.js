...
const internationalCancelBtn = document.querySelector('.internationalCancelBtn');
...
const paymentMethodCancelBtn = document.getElementById('paymentMethodCancelBtn');
...
function updateTimer() {
    if (time > 0) {
        ...
        countdownInt.innerText = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
    } else {
    clearInterval(timer);
    timer = null;
...
function resetTimer() {
    clearInterval(timer);
    timer = null;
    time = 1000; // Reset time to seconds
